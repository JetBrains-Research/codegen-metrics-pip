...
from ..tokenizers.tokenizer_code import TokenizerCode
...
class BLEU(Metric):
    ...
    def __init__(
        ...
        # Sanity check
        assert self.smooth_method in self.SMOOTH_DEFAULTS.keys(), "Unknown smooth_method {self.smooth_method!r}"
        ...
        if tokenize is None:
            ...
            if self.trg_lang == "ja" and best_tokenizer != "ja-mecab":
                sacrelogger.warning("You should use the 'ja-mecab' tokenizer for Japanese.")

        # Create the tokenizer
        self.tokenizer = TokenizerCode()
    ...
    def compute_bleu(
        ...
        """
        assert smooth_method in BLEU.SMOOTH_DEFAULTS.keys(), "Unknown smooth_method {smooth_method!r}"
        ...
        # Compute BLEU score
        score = bp * math.exp(sum([my_log(p) for p in precisions[:eff_order]]) / eff_order)
    ...
    def _compute_segment_statistics(self, hypothesis: str, ref_kwargs: Dict) -> List[int]:
        ...
        hyp_ngrams, hyp_len = extract_all_word_ngrams(hypothesis, 1, self.max_ngram_order)
    ...
    def sentence_score(self, hypothesis: str, references: Sequence[str]) -> BLEUScore:
        ...
        if not self.effective_order:
            sacrelogger.warning("It is recommended to enable `effective_order` for sentence-level BLEU.")
